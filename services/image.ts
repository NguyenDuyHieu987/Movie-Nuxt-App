...
export async function makeImageRequest(url: string, options: AxiosRequestConfig = {}) {
...
export function getImage(path: string, type: string, crop: string = '') {
  ...
  if (crop.length == 0 || !nuxtConfig.app.production_mode) { return `${URL_API_IMAGE1}/images/${type}/${path}` }
...
export function getServerImage(path: string, type: string, crop = '') {
  ...
  const URL_API_IMAGE1 = isProduction ? URL_API_IMAGE : 'http://localhost:5002/static'

  if (crop.length == 0 || !isProduction) return `${URL_API_IMAGE1}/images/${type}/${path}`
