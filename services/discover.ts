...
export function getMoviesByGenres(
  ...
  const genre: genre | undefined = getGenreByShortName(genre_short_name, ALLGENRES);

  return makeRequest(
    `/discover/all?sort_by=${sort_by}&with_genres=${genre!.id}&page=${page}&limit=${limit}`
  );
}
...
export function getMovieByCountry(
  ...
  const country: country | undefined = getCountryByShortName(country_short_name, ALLCOUNTRIES);
