...
const currentImage = ref<string>(getImage(props.data[0]?.backdrop_path, 'backdrop', 'h-250'));
...
watchEffect(() => {
  if (props.data) {
    currentImage.value = getImage(props.data[0]?.backdrop_path, 'backdrop', 'h-250');
...
const handleArrows = (scrollVal: number) => {
  const maxScrollableWidth = slider.value!.scrollWidth - slider.value!.clientWidth;
...
const onPointerOverSlider = (e: any) => {
  if (e.target.closest('.slider-item')) {
    ...
    const sliderImage = sliderItem.querySelector('.img-box img') as HTMLImageElement;

    if (sliderImage != null) {
      currentImage.value = sliderImage.src;
    }
    const indexItem = sliderItem.querySelector('.movie-card-item').getAttribute('index');
